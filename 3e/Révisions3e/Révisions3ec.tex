...
cross/.default={1pt}}
	...
	\renewcommand{\it}{Retour au sommaire : page \ref{init}
	\subsection{Calcul $ \mathbf{ \then}.$}\label{\then} \stepcounter{n} }
	\newcommand{\itt}{Retour au sommaire : page \ref{init}
	\subsection{Problème $\mathbf{\alph{n}}$}\label{p \then}
	\stepcounter{n}}
...
\section{Avant-propos}\label{init}
...
\item Partie \ref{pbs} de a). à k). et q). à w).
\end{itemize} 
...
\itt
Quel nombre faut-il ajouter aux deux termes de la fraction $\frac7{13}$ pour qu'elle devienne égale à $\frac23$ ? 
\begin{sol}
...
\end{sol}
 ...
 \itt Quel nombre faut-il retrancher aux deux termes de la fraction $\frac{17}{23}$ pour obtenir une fraction égale à $\frac58$ ? 
 \begin{sol}
\textbf{Mise en équation :}
Notons $x$ le nombre cherché. L'énoncé devient
...
\end{sol}
 ...
 \itt Quel nombre faut-il ajouter aux deux termes de la fraction $\frac58$ et retrancher aux deux termes de la fraction $\frac34$ pour obtenir deux fractions égales ? \begin{sol}
\textbf{Mise en équation :}
Notons $x$ le nombre cherché. L'énoncé devient
...
\end{sol}
 ...
 \itt Une personne dispose de deux heures pour effectuer une promenade. Elle part en tramway à la vitesse moyenne de $12$ km à l'heure et revient à pied, à la vitesse moyenne de $4$ km à l'heure. À quelle distance du point de départ devra-t-elle quitter le tramway ? 
 \begin{sol}
...
\[x=24\div4 = 6.\]On doit donc descendre après $6$ kilomètres. \\
 ...
 \end{sol}
 \itt Un épicier achète un certain nombre de bouteilles de vin pour $225$ euros. En vendant la bouteille $1,65$ euros, il gagnerait autant que ce qu'il perdrait s'il la vendait $1,35$ euros. Quel est le nombre des bouteilles de vins ? 
 \begin{sol}
...
Le bénéfice dans le premier cas est égal à la perte dans le second ce qui était la condition voulue.
 \end{sol}
 \itt On considère un polygone régulier à $6$ côtés.
 \begin{enumerate}
...
\end{enumerate}   
\begin{sol}
(a) \& (b) Voici la figure demandée aux deux premières questions : 
\[\begin{tikzpicture}
\draw(0,0)--(0:3)--(60:3)--(0,0);
\draw(0,0)--(60:3)--(120:3)--(0,0);
\draw(0,0)--(120:3)--(180:3)--(0,0);
\draw(0,0)--(180:3)--(240:3)--(0,0);
\draw(0,0)--(240:3)--(300:3)--(0,0);
\draw(0,0)--(300:3)--(360:3)--(0,0);
\draw (30:.5) node{O};
\draw(0:3.3) node{A};
\draw(60:3.3) node{B};
\draw(120:3.3) node{C};
...
\draw(300:3.3) node{F};
\end{tikzpicture}\]
(c) Par construction, les six triangles $AOB$, $BOC$, $COD$, $DOE$ $EOF$ et $FOA$ sont égaux, et on a $OA=OB=OC=OD=OE=OF$, donc ces six triangles sont isocèles en $O$. Il suffit donc de montrer que $OAB$ a trois angles de $60$ degrés. 
Les six angles $\widehat{AOB}$, $\widehat{BOC}$, $\widehat{COD}$, $\widehat{DOE}$ $\widehat{EOF}$ et $\widehat{FOA}$ sont de même mesure et consécutivement adjacents. Leur réunion et l'angle complet de $360^o$. On en déduit que \begin{eqnarray*} 
\widehat{AOB}+\widehat{BOC}+\widehat{COD}+\widehat{DOE}+\widehat{EOF}+\widehat{FOA}&=& 360^o\\
6\times \widehat{AOB} &=& 360^o\\
\widehat{AOB} &=& 360^o\div 6 \\
\widehat{AOB} &=& 60^o\end{eqnarray*}
...
\begin{eqnarray*} 180^o &=& \widehat{AOB} + \widehat{ABO} + \widehat{BAO} \\
180^o &=& 60^o + \widehat{ABO} + \widehat{ABO} \\
180^o -60^o &=& 2\times \widehat{ABO}\\
120^o &=& 2\times \widehat{ABO}\\
120^o\div 2 &=& \widehat{ABO}\\
60^o&=& \widehat{ABO}\end{eqnarray*}
et donc $\widehat{ABO}=\widehat{BAO}=60^o$. 

Le triangle $AOB$ (et donc les cinq autres triangles qui lui sont égaux) a donc trois angles de $60^o$. Ils sont donc d'ailleurs équilatéraux. 
...
\end{sol}
 ...
 \itt On considère un polygone régulier à $5$ côtés.
 \begin{enumerate}
 \item Réaliser une figure.
 \item Relier tous les sommets au centre $O$. 
...
\end{enumerate}   
\begin{sol}Voici la figure demandée aux deux premières questions : 
\[\begin{tikzpicture}
\draw(0,0)--(0:3)--(72:3)--(0,0);
\draw(0,0)--(72:3)--(144:3)--(0,0);
\draw(0,0)--(144:3)--(216:3)--(0,0);
\draw(0,0)--(216:3)--(288:3)--(0,0);
\draw(0,0)--(288:3)--(360:3)--(0,0);
\draw (36:.5) node{O};
\draw(0:3.3) node{A};
\draw(72:3.3) node{B};
\draw(144:3.3) node{C};
...
\draw(288:3.3) node{E};
\end{tikzpicture}\]
(c) Par construction, les cinq triangles $AOB$, $BOC$, $COD$, $DOE$ et $EOA$ sont égaux, et on a $OA=OB=OC=OD=OE$, donc ces cinq triangles sont isocèles en $O$. Il suffit donc de calculer les angles de $OAB$. 
Les cinq angles $\widehat{AOB}$, $\widehat{BOC}$, $\widehat{COD}$, $\widehat{DOE}$ $\widehat{EOA}$ sont de même mesure et consécutivement adjacents. Leur réunion et l'angle complet de $360^o$. On en déduit que \begin{eqnarray*} 
\widehat{AOB}+\widehat{BOC}+\widehat{COD}+\widehat{DOE}+\widehat{EOA}&=& 360^o\\
5\times \widehat{AOB} &=& 360^o\\
\widehat{AOB} &=& 360^o\div 5 \\
\widehat{AOB} &=& 72^o\end{eqnarray*}
Enfin, le triangle $AOB$ est isocèle en $O$, donc ses angles de base $\widehat{ABO}$ et $\widehat{BAO}$ sont de même mesure. Écrivant alors la somme des trois angles de ce triangle, il vient 
\begin{eqnarray*} 180^o &=& \widehat{AOB} + \widehat{ABO} + \widehat{BAO} \\
180^o &=& 72^o + \widehat{ABO} + \widehat{ABO} \\
180^o -72^o &=& 2\times \widehat{ABO}\\
108^o &=& 2\times \widehat{ABO}\\
108^o\div 2 &=& \widehat{ABO}\\
54^o&=& \widehat{ABO}\end{eqnarray*}
et donc $\widehat{ABO}=\widehat{BAO}=54^o$. 

Le triangle $AOB$ (et donc les quatre autres triangles qui lui sont égaux) a donc un angle au sommet de $72^o$ et deux angles de base de $54^o$. 
...
\end{sol}
\itt Utiliser les deux exercices précédents pour généraliser à un polygone régulier avec un nombre quelconque $N$ de côtés. On montrera ainsi que les angles d'une telle figure valent tous $\frac{N-2}N\times 180^o$.
\begin{sol}
En adaptant les deux exercices précédents, notons $AOB$ le triangle obtenu en joignant un côté $[AB]$ du polygone régulier à $N$ côtés à son centre. Le triangle $AOB$ est isocèle en $O$. 

L'angle $\widehat{AOB}$ mesure un $N$-ème de tour complet, 
donc $\frac{360^o}N$. 

Enfin, le triangle $AOB$ est isocèle en $O$, donc ses angles de base $\widehat{ABO}$ et $\widehat{BAO}$ sont de même mesure. Écrivant alors la somme des trois angles de ce triangle, il vient 
\begin{eqnarray*} 180^o &=& \widehat{AOB} + \widehat{ABO} + \widehat{BAO} \\
180^o &=& \frac{360^o}N + \widehat{ABO} + \widehat{ABO} \\
180^o -\frac{360^o}N &=& 2\times \widehat{ABO}\\
180^o -\frac{360^o}N &=& 2\times \widehat{ABO}\\
\left(180^o -\frac{360^o}N\right)\div 2 &=& \widehat{ABO}\\
90^o- \frac{180^o}N&=& \widehat{ABO}\\
\frac{N}N\times90^o- \frac2N\times 90^o &=& \widehat{ABO}\\
\left(\frac{N}N-\frac2N\right)\times 90^o &=& \widehat{ABO}\\
\frac{N-2}N\times 90^o &=& \widehat{ABO}
\end{eqnarray*}
donc le triangle $AOB$ (ainsi que les $N-1$ autres triangles qui forment ce polygone) a des angles de base de $\frac{N-2}N\times 90^o$. 

Enfin, les angles du polygone mesurent tous comme l'angle
$\widehat{ABC}$ qui est la réunion des angles adjacents $\widehat{ABO}$ et $\widehat{OBC}$ qui mesurent donc chacun $\frac{N-2}N\times 90^o$. On en déduit que \begin{eqnarray*}
\widehat{ABC} &=& \widehat{ABO}+\widehat{OBC}\\
&=& \frac{N-2}N\times 90^o+\frac{N-2}N\times 90^o\\
&=&  \frac{N-2}N\times(90^o+90^o)\\
&=& \frac{N-2}N\times 180^o\end{eqnarray*}
 ce qui était le résultat attendu : les angles d'un polygone régulier à $N$ côtés mesurent tous $\frac{N-2}N\times 180^o$.

En particulier, on se rassure avec les quatre cas particuliers connus : \begin{itemize}
\item pour $N=3$, on trouve $\frac{3-2}3\times 180^o = \frac13\times180^o = 60^o$, ce qui correspond aux angles d'un triangle équilatéral ;
\item pour $N=4$, on trouve $\frac{4-2}4\times 180^o = \frac24\times180^o = 90^o$, ce qui correspond aux angles d'un carré;
\item pour $N=5$, on trouve $\frac{5-2}5\times 180^o = \frac35\times180^o = 108^o$, ce qui correspond aux angles d'un pentagone régulier ;
\item pour $N=6$, on trouve $\frac{6-2}6\times 180^o = \frac46\times180^o =120^o$, ce qui correspond aux angles d'un hexagone régulier.
\end{itemize}
Notons qu'on en tirerait donc après calcul que : \begin{itemize}
\item l'octogone régulier a des angles de $\frac{8-2}8\times 180^o = 135^o$,
\item l'ennéagone régulier a des angles de $\frac{9-2}9\times 180^o = 140^o$,
\item le décagone régulier a des angles de $\frac{10-2}{10}\times180^o = 144^o$
\end{itemize}
Attention, certaines valeurs peuvent tomber moins juste : 
\begin{itemize}
\item  pour l'heptagone régulier ($N=7$), on trouverait des angles de $\frac{7-2}{7}\times 180^o = \frac57\times 180^o$, qui n'est pas un nombre décimal (son développement décimal est $128,571428~571428~571428~571428\ldots$ degrés).
\item  pour l'hendécagone régulier ($N=11$), on trouverait des angles de $\frac{11-2}{11}\times 180^o = \frac9{11}\times 180^o$, qui n'est pas un nombre décimal (son développement décimal est $147,272727\ldots$ degrés).
\end{itemize}
\end{sol}
\itt On trace deux points $A$ et $B$ et une droite $(d)$ sur une feuille. Expliquer comment construire le cercle passant par $A$ et $B$ dont le centre appartient à $(d)$. Existe-t-il toujours ? Y en a-t-il toujours un seul ?
\begin{sol}
Ce problème revient à trouver le centre $O$ d'un tel cercle, le rayon étant alors simplement $OA$. Le centre doit vérifier les deux conditions $OA=OB$ et $O\in(d)$.

La première de ces conditions revient à dire que $O$ appartient à la médiatrice du segment $[AB]$. On cherche donc un point qui appartienne à la fois à $(d)$ et à cette médiatrice. Trois cas de figure se présentent alors : 
\begin{itemize}
\item Premier cas : $(d)$ est la médiatrice du segment $[AB]$. À ce moment, tout point de $(d)$ convient et permet de tracer un cercle passant par $A$ et $B$. Il y a donc une infinité de tels cercles. 
\item Deuxième cas : $(d)$ est parallèle à la médiatrice du segment $[AB]$, il n'y a alors aucun point d'intersection et donc aucun cercle solution du problème. 
\item Troisième et dernier cas : sinon, $(d)$ coupe la médiatrice de $[AB]$ en un unique point $O$, qui est le centre du seul cercle possible. Dans ce cas, on peut alors construire effectivement le point $O$ en traçant la médiatrice de $[AB]$ à la règle et au compas et en regardant où elle rencontre $(d)$.
\end{itemize}
\end{sol}
\itt On place trois points $A$, $B$, $C$ sur une feuille. À quelle condition passe-t-il un cercle par ces trois points ? Comment le construire ? 
\begin{sol}
Pour qu'un tel cercle existe, si on note $O$ son centre et $R$ son rayon, il faut et il suffit que $OA=OB=OC=R$. Le point $O$ doit donc être à la même distance des trois sommets. 

Considérons les médiatrices de $[AB]$ et $[AC]$. 
\begin{itemize}
\item Si $A$, $B$ et $C$ sont alignés, ces deux droites sont perpendiculaires à la droite passant par ces trois points, donc elles sont parallèles entre elles. Mais alors, comme $OA=OB$, et $OA=OC$, $O$ devrait être sur les deux médiatrices, ce qui est impossible. Aucun cercle ne passe donc par trois points alignés.
\item Sinon, elles se coupent en un point $O$. Ce point est sur la médiatrice de $[AB]$ donc $OA=OB$, et il est sur la médiatrice de $[AC]$ donc $OA=OC$. On a donc $OA=OB=OC$. En prenant cette distance commune comme rayon, on obtient le cercle voulu. 
\end{itemize}
...
\end{sol}
\itt On considère un quadrilatère convexe $ABCD$ dans lequel la médiatrice de 
...
\begin{sol}
\begin{enumerate}
\item On sait que $O$ appartient à la médiatrice de $[AB]$. Or, cette médiatrice est l'ensemble des points équidistants de $A$ et $B$, donc $OA=OB$. 

De même, comme $O$ appartient à la médiatrice de $[CD]$ (qui est la même droite que la précédente), $OC=OD$. 

De plus, comme $O$ appartient à la médiatrice de $[BC]$, $OB=OC$.

On a donc bien $OA=OB=OC=OD$, ce qui garantit que $O$ est à la même distance des quatre sommets. Le cercle de centre $O$ et de rayon $OA$ passe donc par $A$, $B$, $C$ et $D$. 
\item Étant donné que $OA=OD$, $O$ est à la même distance des deux extrémités de $[AD]$. Or, un point équidistant des extrémités d'un segment est sur sa médiatrice, donc $O$ appartient à la médiatrice de $[AD]$. 

On procède de même pour le reste de la question en remarquant que $OA=OC$ et que $OB=OD$. 
\end{enumerate}
\end{sol}
\itt Tracer un parallélogramme $ABCD$, et les bissectrices de ses quatre angles. Elles se croisent intérieurement au parallélogramme en quatre points $E$, $F$, $G$ et $H$. Que dire du quadrilatère $EFGH$ formé ? Le démontrer. 
\begin{sol}
On peut illustrer le problème avec la figure suivante : 

\begin{figure}[H]\center
\begin{tikzpicture}
\draw(0,0) -- (120:2) -- ++(3,0) -- (3,0)--(0,0);
\draw[dashed] (0,0) --(60:3) (120:2) -- ++(-30:3)
(120:2) ++(3,0) -- ++(-120:3) (3,0) -- ++(150:3);
\draw (0,0) ++(-.25,0) node{A} (120:2) ++(-.25,0) node{B};
\draw (3.25,0) node{D} ++ (120:2) node {C};
\draw (.21,.9) node{E} ++ (.3,.4) node{F};
\draw (1.4, -.4) ++(.21,.9) node{H} ++ (.3,.4) node{G};
\end{tikzpicture}
\end{figure}

Montrons donc que le quadrilatère central $EFGH$ est un rectangle. On veut montrer que les quatre angles sont droits. On peut se contenter de montrer que l'angle $\widehat{FEH}$ est droit, les arguments étant identiques pour les trois autres angles.

On sait que $ABCD$ est un parallélogramme, donc les angles $\widehat{ABC}$ et $\widehat{DAB}$ sont supplémentaires.\footnote{Propriété classique des parallélogrammes. Sinon, cela se redémontre avec les angles alternes-internes.} : 
\[ \widehat{ABC}+\widehat{DAB}=180^o\]
On sait que $[BE)$ est la bissectrice de $\widehat{ABC}$, donc $\widehat{ABE}= \frac12\times \widehat{ABC}$. 

De même, comme $[AE)$ est la bissectrice de $\widehat{DAB}$, on a $\widehat{EAB} =\frac12\times \widehat{DAB}$.

De plus, la somme des angles du triangle $ABE$ est $180$ degrés, donc 
 \[\widehat{AEB}= 180^o-\widehat{EAB}-\widehat{ABE}\]
 Enfin, $\widehat{FEH}$ et $\widehat{AEB}$ sont opposés par le sommet donc égaux.
 
On a donc \begin{eqnarray*}
\widehat{FEH} &=& \widehat{AEB}\\
&=& 180^o-\widehat{EAB}-\widehat{ABE}\\
&=& 180^o - \frac12\times \widehat{DAB}- \frac12\times\widehat{ABC}\\
&=& 180^0-\frac12\times(\widehat{DAB}+\widehat{ABC}) \\
&=& 180^o - \frac12 \times 180^o\\
&=& 180^o- 90^o\\
&=& 90^o\end{eqnarray*} ce qui montre que l'angle $\widehat{FEH}$ est droit. On procéderait de même pour montrer que les trois autres angles de $EFGH$ sont également droits. C'est donc un rectangle.
\end{sol}
\itt On trace un triangle $ABC$ et on place les milieux $D$, $E$, $F$ de ses trois côtés. Montrer que le triangle $DEF$ est une réduction du triangle $ABC$ dont on déterminera le coefficient. 
\begin{sol}
Illustrons par la figure suivante : 
\begin{figure}[H]\center
\begin{tikzpicture}
\draw(0,0) -- (4, 0) -- (3, 4) --(0,0) ;
\draw (-.25, -.25) node{A} (4.25, -.25) node{B} (3.25, 4.25) node{C};
\draw(2,0) -- (1.5, 2) -- (3.5, 2) --(2,0);
\draw(1.15, 2) node{B'} (3.85, 2) node{A'} (2,-.25) node{C'};
\end{tikzpicture}
\end{figure}

Par définition les points $A'$, $B'$ et $C'$ sont les milieux respectifs de $[BC]$, $[AC]$ et $[AB]$. 

On sait que : \begin{itemize}
\item $B'$ appartient à $[AC]$,
\item $A'$ appartient à $[BC]$, 
\item D'une part, $\frac{CB'}{CA} = \frac12$ car $B'$ est le milieu de $[AC]$. 
D'autre part, $\frac{CA'}{CB} = \frac12$ car $A'$ est le milieu de $[BC]$.
\end{itemize}
D'après la réciproque du théorème de Thalès, on a donc $(A'B')//(AB)$.

On peut alors appliquer le théorème de Thalès, qui montre que $\frac{A'B'}{AB}=\frac{CA'}{CB}=\frac{CB'}{CA}$. Ces deux derniers rapports valant $\frac12$, on a donc $\frac{A'B'}{AB}=\frac12$, et la longueur $A'B'$ est la moitié de la longueur $AB$. 

On montrerait avec les mêmes arguments que $A'C'$ est la moitié de $AC$ et que $B'C'$ est la moitié de $BC$. Il s'ensuit que le triangle $A'B'C'$ a des longueurs qui sont les moitiés des longueurs de $ABC$. Ces deux triangles sont donc semblables avec coefficient de similitude $\frac12$.
\end{sol}

\itt Dans le même contexte que l'exercice précédent, on relie chaque milieu au sommet opposé, et on note $G$ le point d'intersection des trois segments ainsi formés. Montrer que l'homothétie de centre $G$ et de rapport $-2$ envoie chaque sommet du triangle $DEF$ sur un sommet du triangle $ABC$. 

\begin{sol}
On enrichit la figure de l'exercice précédent : 
Illustrons par la figure suivante : 
\begin{figure}[H]\center
\begin{tikzpicture}
\draw(0,0) -- (4, 0) -- (3, 4) --(0,0) ;
...
\draw(2,0) -- (1.5, 2) -- (3.5, 2) --(2,0);
\draw(1.15, 2) node{B'} (3.85, 2) node{A'} (2,-.25) node{C'};
\draw[dashed] (0,0)-- (3.5,2) (2,0)--(3,4) (1.5,2)--(4,0);
\draw (2.58, 1.3) node{G};
\end{tikzpicture}
\end{figure}
On admet que les trois médianes $[AA']$, $[BB']$ et $[CC']$ sont concourantes en un point $G$, et que ce point est situé aux deux tiers de chaque médiane de telle sorte que $BG= 2B'G$, $AG=2A'G$ et $CG=2C'G$. C'est exactement dire que l'homothétie de centre $G$ et de rapport $-2$ envoie $A'B'C'$ sur $ABC$, ou, de manière équivalente, que celle de centre $G$ et de rapport $-\frac12$ envoie $ABC$ sur $A'B'C'$. 

Évidemment toute la difficulté est dans le résultat ici admis, dont on peut trouver une démonstration ici à partir de la page 15 : \url{https://leturcqd.github.io/ressources/5e/Cours/G\%C3\%A9om\%C3\%A9trie\%20du\%20triangle.pdf#page=15} 
\end{sol}
\itt Dans un triangle $ABC$, on trace la hauteur issue de $A$ qui rencontre\footnote{Attention, la hauteur coupe le segment, c'est une vraie hypothèse : si elle rencontrait $(BC)$ hors du segment, le résultat serait faux !} $[BC]$ en $H$. On suppose $AB^2 = BH \times BC$. Montrer que $ABC$ est rectangle en $A$.

\begin{sol}

Posons la figure suivante, où l'angle en $H$ est droit : 
\begin{figure}[H]
\center
\begin{tikzpicture}
\draw (0,0) -- (5,0) -- (53:3)-- (0,0);
\draw (53:3) -- (1.8,0);
\draw (-.25,-.25) node{B} (1.8, -.3) node{H} (5.25, -.25) node{C} (53:3) ++ (.25, .15) node{A};
\end{tikzpicture}
\end{figure}
Comme on sait que l'angle en $H$ est droit, on peut écrire le théorème de Pythagore dans les deux triangles rectangles $AHB$ et $AHC$, ce qui donne : 
\[ AH^2+BH^2 = AB^2 \]
et \[AH^2+CH^2=AC^2\]

On veut montrer que le triangle $ABC$ est rectangle. Étant donné les outils disponibles, le seul moyen raisonnable est d'obtenir la relation de Pythagore $AC^2+AB^2 = BC^2$ astucieusement. 

Comme $H\in[BC]$, on a $BC= BH+CH$, et \[BC^2 = (BH+CH)^2 = BH^2+2\times BH\times CH + CH^2.\]
Mais \begin{eqnarray*} BH\times CH &=& BH \times (BC-BH)\\
&=& BH\times BC - BH\times BH\\
&=& AB^2 - BH^2 \text{         car $AB^2=BH\times BC$} \\
&=& AH^2\end{eqnarray*}

Donc \[BC^2 = BH^2 + 2\times AH^2 + CH^2\]
D'autre part, \begin{eqnarray*} AB^2+AC^2 &=& AH^2+BH^2 + AH^2+CH^2\\
2AH^2+BH^2+CH^2 \\
&=& BH^2 + 2\times AH^2 + CH^2
\end{eqnarray*}
Donc $AB^2+AC^2 = BC^2$. D'après la réciproque du théorème de Pythagore, le triangle $ABC$ est donc rectangle en $A$. 
\end{sol}

 
\itt On considère un quadrilatère $ABCD$. Montrer que les milieux des côtés forment un  parallélogramme.
\begin{sol}
Commençons avec la figure suivante : 

\begin{figure}[H]
\center
\begin{tikzpicture}
\draw (0,0) -- (8, 0) -- (8, 2) -- (2, 4) -- (0,0);
\draw(4,0) -- (8,1) -- (5,3) -- (1,2)--(4,0);
\draw (-.25, -.25) node{A} (4, -.35) node{I} 
(8.25, -.25) node{B} (8.35, 1) node{J} (8.25, 2.25) node{C} (5.25, 3.25) node{K} (1.75, 4.25) node{D}
(.75, 2.1) node{L} ;
\draw[dashed] (0,0) -- (8,2) (2,4) -- (8,0);
\end{tikzpicture}
\end{figure}
Montrons que $(IJ) //(KL)$. Pour ce faire, on va montrer séparément que $(IJ)$ et $(KL)$ sont parallèles à la diagonale $(AC)$. 

On applique la réciproque du théorème de Thalès dans le triangle ABC. On sait que : \begin{itemize}
\item $I$ appartient à $[AB]$;
\item $J$ appartient à $[BC]$;
\item D'une part, comme $I$ est le milieu de $[AB]$, $\frac{BI}{BA}=\frac12$. 

D'autre part, comme $J$ est le milieu de $[BC]$, $\frac{BJ}{BC}=\frac12$.

Donc $\frac{BI}{BA} = \frac{BJ}{BC}$. 
\end{itemize}
D'après la réciproque du théorème de Thalès, les droites $(IJ)$ et $(AC)$ sont donc parallèles. 

De la même manière, en utilisant le triangle $ACD$, on montre à l'aide de la réciproque du théorème de Thalès que $(LK)$ et $(AC)$ sont parallèles. 

Enfin, on sait que les droites $(IJ)$ et $(LK)$ sont parallèles à la même droite $(AC)$, donc elles sont parallèles entre elles. 

De la même manière, à l'aide des triangles $ABD$ et $BDC$, on montrerait que $(IL)$ et $(JK)$ sont toutes deux parallèles à $(BD)$ et sont donc parallèles entre elles. 

Le quadrilatère $IJKL$ a ses côtés opposés parallèles, c'est donc un parallélogramme. 

\end{sol}

 \itt Trouver trois nombres entiers consécutifs (consécutifs : qui se suivent) dont la somme est égale à $57$. 
 \begin{sol}
...
\textbf{Résolution  de l'équation :} On réduit l'expression obtenue qui devient : \[3x+3 = 57\]
 ...
 \end{sol}
 \itt La somme de cinq nombres impairs consécutifs est 85. Quels sont ces nombres ? 
  \begin{sol}
...
c'est-à-dire \[5x+20 = 85\]
 ...
 \end{sol}
 \itt Un père a 29 ans ; son fils a 5 ans ; dans combien d'années l'âge du père sera-t-il le triple de l'âge du fils ? 
 \begin{sol}
...
On doit donc attendre $N=7$ années.\\
 ...
 \end{sol}
 \itt Un père a 41 ans ; ses trois enfants sont âgés de 7, 9 et 13 ans. Dans combien d'années l'âge du père sera-t-il égal à la somme des âges de ses enfants ? 
 \begin{sol}
 \textbf{Mise en équation :} 
...
Il faut donc attendre $6$ ans.\\
 ...
 \end{sol}
 \itt Trouver un nombre dont la somme des quotients par 5, 7 et 9 soit égale à 429. 
 \begin{sol}
...
Le nombre recherché est donc forcément $945$.\\
 ...
 \end{sol}
 \itt Développer les produits suivants : 
 \[ (x-1)(x+1)\]
 \[(x-1)(x^2+x+1)\]
 \[(x-1)(x^3+x^2+x+1)\]
...
En général, on en tire : 
 ...
 \end{sol}
 \itt Un cercle délimite deux régions du plan : l'intérieur et l'extérieur. Deux cercles peuvent en déterminer quatre. S'en convaincre par un dessin. Que dire du nombre maximal de régions délimitées par trois cercles ? Que se passe-t-il pour quatre ? Trouver une formule générale.
 
\begin{sol}

Notons $R(n)$ le nombre maximal de régions délimitées par $n$ cercles.
\[\begin{tikzpicture}
\draw (0,0) circle (2);
\draw (0,0) node{$1$};
\draw (3,0) node{$2$};
...
\draw(180:3) node{8};
\end{tikzpicture}\]
Les figures précédentes laissent deviner que l'on a $R(1) =2$, $R(2)=4$ et $R(3)=8$. Il reste à montrer qu'elles sont bien optimales (i.e. qu'on ne peut pas faire plus de régions avec ce nombre de cercles). 

On pourrait en inférer rapidement que chaque nouvelle figure comporte deux fois plus de région, mais la détermination de l'image suivante montre que c'est faux, voici en effet un exemple maximal pour $n=4$ qui montre que $R(4)$ vaut $14$ et non $16$.  
...
\draw(0,0) node{1};
\draw(90:1.25) node{2};
\draw(180:1.25) node{3};
\draw(270:1.25) node{4};
\draw(0:1.25) node{5};
\draw(135:2) node{6};
\draw(225:2) node{7};
\draw(315:2) node{8};
\draw(45:2) node{9};
\draw(90:2.5) node{10};
\draw(180:2.5) node{11};
\draw(270:2.5) node{12};
\draw(0:2.5) node{13};
\draw(45:3) node{14};
\end{tikzpicture}\]

Pour montrer plus rigoureusement que l'on ne peut pas faire plus de régions qu'ainsi, raisonnons de la manière suivante.\begin{itemize}
\item Un cercle sépare toujours deux zones : l'intérieur et l'extérieur, ce qui montre que $R(1)=2$.
\item Pour tracer deux cercles, il suffit alors de tracer un second cercle. Il croise le premier au maximum en deux points. Ce second cercle est alors découpé en deux arcs de cercle, qui coupent chacun une des deux régions préexistantes en deux. Chaque région ainsi découpée devient donc deux régions, ce qui augmente le nombre de régions de $1$ par zone, c'est-à-dire de deux. Voici une figure : 

\begin{figure}[H]\center\begin{tikzpicture}
\draw (180:1) circle (2); 
\draw[red] (0, 1.94) arc(111.5:-111.5:2);
\draw[blue] (0, 1.94) arc(111.5:248.5:2);
\draw (0,0) node{$1$};
\draw (2,0) node{$2$};
\draw (-2,0) node{$3$};
\draw(4,0) node {$4$};
\end{tikzpicture}
\caption{L'arc bleu coupe l'intérieur du cercle noir en deux zones (1 et 3) et l'extérieur en deux zones (2 et 4), ce qui rajoute bien deux zones.}
\end{figure}
\item De même, lorsqu'on rajoute un $n+1$-ème cercle à la figure, il rencontre chacun des $n$ autres cercles en deux points au plus. Il y a donc au plus $2n$ intersections avec la figure préexistante. Cela délimite sur ce nouveau cercle $2n$ arcs. Chacun de ces $2n$ arcs sépare une région préexistante en deux régions, et augmente donc le nombre de régions de $1$. 

On en déduit que l'ajout d'un cercle à une figure comportant déjà $n$ cercles ajoute au plus $2n$ régions, d'où la relation : 
\[R(n+1) = R(n) + 2n\]
\end{itemize}

Le nombre de régions maximal pour $n$ cercles est donc 
\[R(n) = 2(n-1) + 2(n-2) + \ldots + 2\times 2 + 2\times 1 + \underbrace{R(1)}_{=2}\]
Il s'agit donc de la somme des entiers pairs jusque $2n$ plus $2$. 
On peut remarquer alors que 
\begin{eqnarray*} 2(n-1) &+& 2(n-2) + \ldots + 2\times 2 + 2\times 1 \\&=& 2(  (n-1) + (n-2) + \ldots + 2 + 1 ) \\
&=& ( (n-1) + (n-2) + \ldots + 2 + 1)+ ((n-1) + (n-2) + \ldots + 2 + 1)\\
&=&  (n-1) + 1  + (n-2) + 2 +\ldots + 2 + (n-2) + 1 + (n-1)\\
&=& n + n + \ldots + n + n \\
&=& (n-1)n
\end{eqnarray*}
On trouve alors finalement $R(n)=2+n(n-1)$. 

On peut vérifier la cohérence avec les premières valeurs trouvées : 
\[R(1) = 2+ 1(1-1) = 2+ 1\times 0 = 2+0=2\]
\[R(2) = 2+2(2-1) = 2+2\times 1 = 2+2 = 4\]
\[R(3) = 2+3(3-1)=2+3\times2 = 2+6 = 8\]
\[R(4) = 2+4(4-1) = 2+4\times3 = 2+12=14\]

On peut aussi visualiser cette formule sur la figure suivante (où $n=20$ pour fixer les idées): 

\begin{figure}[H]
\[\begin{tikzpicture}

\foreach \r in {1,...,15}{
    \draw (24*\r:3) circle (5);
  }
\end{tikzpicture}\]
\caption{Une configuration optimale pour $15$ cercles avec ses $212$ zones.}
\end{figure}
On peut alors retrouver la formule $2+n(n-1)$ de la manière suivante : \begin{itemize}
\item Il y a deux zones spéciales : la zone intérieure centrale, et la zone extérieure à tous les cercles. 
\item Il y a $n=15$ bras en spirale, qui sont tous découpés en $n-1=14$ zones. En effet, chaque bras en spirale correspond à l'intervalle entre deux cercles consécutifs, donc il y en a $15$. Chacun contient $14$ zones, car ces zones sont délimitées par les intersections avec les $13$ cercles restants et que $13$ coupures sur un segment délimitent $14$ segments. 
\end{itemize}
\end{sol} 
 
 \itt Si on coupe un disque par une corde, on délimite deux régions. Une deuxième corde délimite alors quatre régions. Généraliser. 
 
 \begin{sol}
 Commençons par tracer les cinq cordes suivantes pour fixer les idées.
\[ \begin{tikzpicture}
 \draw (0,0) circle (3);
 \draw (340:3) -- (143:3);
 \end{tikzpicture}
 ...
 \end{tikzpicture}\]
 \end{sol}
On y voit respectivement $2$, $4$, $7$, $11$ et $16$ zones (attention, certaines sont petites sur la dernière figure). On remarque donc que ce nombre augmente d'abord de $2$, puis de $3$, puis de $4$, puis de $5$, ce qui semble régulier. Montrons que c'est bien le cas systématiquement. 
 Chaque nouvelle corde rencontre les cordes précédentes au plus une fois chacune. Elle a alors au plus $n$ intersections s'il y avait déjà $n$ cordes. Ces $n$ intersections sépare la $n+1$ème corde en $n+1$ segments. Chacun de ces $(n+1)$ segments de corde sépare une zone de la figure précédente en deux nouvelles zones, ce qui augmente leur nombre total de $1$. Globalement, on a donc augmenté le nombre de régions d'au plus $n+1$ quand on a tracé cette $n+1$ème corde. 
 On en déduit donc que le nombre $C(n)$ de régions découpées par $n$ cordes est 
 \[C(n) = \underbrace{n}_{\text{créés par la $n^e$ corde}} + \underbrace{(n-1)}_{\text{créés par la $n-1^e$ corde}} + \ldots +\underbrace{2}_{\text{créés par la $2^e$ corde}} + C(1)\]
 Donc \[C(n) = n+ (n-1) + \ldots + 2 + 2\]
 Le nombre de régions est donc la somme des entiers de $2$ à $n$ augmentée de $2$. 
 On pourrait montrer comme à l'exercice précédent que cela se réécrit \[C(n) = \frac{n(n+1)}2+1.\]
 On retrouve les cinq premières valeurs de début de solution.
 \[C(1) = \frac{1\times2}2+1= \frac22+1 = 1+1=2\]
 \[C(2) = \frac{2\times3}2+1= \frac62+1 = 3+1=4\]
 \[C(3) = \frac{3\times4}2+1= \frac{12}2+1 = 6+1=7\]
 \[C(4) = \frac{4\times5}2+1= \frac{20}2+1 = 10+1=11\]
 \[C(5) = \frac{5\times6}2+1= \frac{30}2+1 = 15+1=16\]
 En particulier, avec $10$ coupures, on aurait $\frac{10\times11}2+1= 57$ parts\footnote{Il est utile de s'amuser à obtenir effectivement les $57$ zones sur un très grand cercle en traçant $10$ cordes.}, et avec $20$ coupures, on aurait $\frac{20\times21}2+1= 211$ parts. 
 
 
 
 \itt Un nombre entier $N$ est dit \emph{pratique} si chaque entier entre $1$ et $N$ peut être écrit comme une somme de diviseurs de $N$ (sans répétition). Par exemple, $6$ est pratique car ses diviseurs sont $1$, $2$, $3$ et $6$ et que : 
 \[ 1 = 1\] 
 \[ 2 = 2 \]
 \[ 3 = 1 + 2 \]
 ...
 \begin{sol}
 Pour chaque nombre, établissons la liste de leurs diviseurs, puis vérifions s'ils permettent d'écrire les nombres voulus.
 \begin{enumerate}
 \item $2$ est pratique car ses diviseurs sont $1$ et $2$, et que \[1=1\]\[2=2\]
 \item $3$ n'est pas pratique, car ses diviseurs sont $1$ et $3$, et que $2$ ne peut pas être écrit en ajoutant $1$ et $3$. 
 \item $4$ est pratique car ses diviseurs sont $1$, $2$ et $4$ et que 
 \[1=1\]
 \[2=2\]
 \[3=1+2\]
 \[4=4\]
 \item $5$ n'est pas pratique, car ses diviseurs sont $1$ et $5$ ce qui ne permet pas de former $2$ (ou $3$ ou $4$).
 \item $6$ est pratique (cf énoncé). 
 \item $7$, $11$, $13$, $17$, $19$, $23$, et $29$ ne sont pas pratiques car ils sont premiers, et que leurs deux seuls diviseurs, $1$ et eux-mêmes ne permettent pas d'écrire $2$, par exemple. 
 \item $8$ est pratique, car ses diviseurs sont $1$, $2$, $4$ et $8$ et que 
 \[1=1\]
 \[2=2\]
 \[3=1+2\]
 \[4=4\]
 ...
 \[7=1+2+4\]
 \[8=8\]
 \item $9$ n'est pas pratique, car ses diviseurs sont $1$,$3$ et $9$, qui ne permettent pas d'écrire $2$. La même méthode montre qu'aucun nombre impair n'est pratique car $2$ ne peut pas être écrit comme somme de $1$ et d'un autre nombre. 
 \item $10$ admet comme diviseurs 1, 2, 5 et 10. On peut écrire 
 \[1=1\]
 \[2=2\]
 \[3=1+3\]
 mais on ne parviendra pas à écrire $4$ comme une somme de 1, 2, 5 et 10 sans répétition.
 \item $12$ admet comme diviseurs 1, 2, 3, 4, 6 et 12 et on a \[1=1\]
 \[2=2\]
 \[3=1+2=3\]
 \[4=1+3=4\]
 \[5=1+4\]
 ...
 \[10 = 4+6\]
 \[11=1+4+6\]
 \[12=12\]donc il est pratique. 
 \item $14$ admet comme diviseurs 1, 2, 7 et 14, ce qui ne permet pas d'obtenir $4$, par exemple. 
 \item 16 est pratique car ses diviseurs sont 1, 2, 4, 8 et 16 et que 
 \[1=1\]
 \[2=2\]
 \[3=1+2\]
 \[4=4\]
 ...
 \[15=1+2+4+8\]
 \[16=16\]
 \item 18 a comme diviseurs 1, 2, 3, 6, 9 et 18. On peut donc former 
 \[1=1\]\[2=2\]\[3=3\]\[4=1+4\]\[5=2+3\]\[6=6\]\[7=1+6\]\[8=2+6\]\[9=9\]\[10=1+3+6\]
 \[11=2+3+6\]\[12= 1+2+3+6\]
 \[13= 1+3+9\]
 \[14=2+3+9\]
 \[15=6+9\]
 \[16=1+6+9\]
 \[17=2+6+9\]
 \[18=18\]donc 18 est pratique.
 \item 20 a comme diviseurs 1, 2, 4, 5, 10 et 20. On peut former 
 \[1=1\]\[2=2\]\[3=1+2\]\[4=4\]\[5=5\]
 \[6=1+5\]\[7=2+5\]\[8=1+2+5\]
 \[9=4+5\]
 \[10=10\]\[11=1+10\]\[12=2+10\]
 \[13=1+2+10\]
 \[14=4+10\]
 \[15=5+10\]
 \[16=2+4+10\]
 ...
 \[18=1+2+5+10\]
 \[19=4+5+10\]
 \[20=20\]donc $20$ est pratique.
 \item 22 a comme diviseurs 1, 2, 11 et 22, donc on ne pourra pas former 4 en les additionnant, et 22 n'est pas pratique.
 \item 24 a comme diviseurs 1, 2, 3, 4, 6, 8, 12 et 24 donc on peut former 
 \[1=1\]\[2=2\]\[3=3\]\[4=4\]\[5=1+4\]\[6=6\]\[7=1+6\]\[8=8\]\[9=1+8\]\[10=2+8\]\[11=1+2+8\]\[12=4+8\]\[13=1+4+8\]\[14=6+8\]\[15=1+6+8\]\[16=2+6+8\]\[17=1+2+6+8\]\[18=6+12\]\[19=1+6+12\]\[20=8+12\]\[21=1+8+12\]\[22=2+8+12\]\[23=1+2+8+12\]\[24=24\] ce qui montre que 24 est pratique. 
 \item 26 a comme diviseurs 1, 2, 13 et 26 avec lesquels on ne peut former 4, donc 26 n'est pas pratique. 
 \item 28 a comme diviseurs 1, 2, 4, 7, 14 et 28 donc on peut former 
 \[1=1\]
 \[2=2\]
 \[3=1+2\]
 \[4=4\]
 ...
 \[6=2+4\]
 \[7=1+2+4\]
 mais on ne peut pas former 8, donc 28 n'est pas pratique. 
 \item Enfin, 30 a comme diviseurs 1, 2, 3, 5, 6, 10, 15 et 30 avec lesquels on peut former \[1=1\]
 \[2=2\]
 \[3=3\]
 \[4=1+3\]
 \[5=2+3\]
 ...
 \[29=1+2+5+6+15\]
 \[30=30\]
 donc 30 est pratique. 
 
 \end{enumerate}On en déduit que les nombres pratiques jusque $30$ sont \[1, 2, 4, 6, 8,
 12,  16, 18, 20, 24, 30\]
 
 Reste à montrer que les puissances de 2 sont pratiques. Tout nombre admet une unique écriture en base 2, sous la forme 
 \[  a.1 + b.2^1 + c.2^2+ d 2^3+ e 2^4+...\]
 Cela montre en particulier que tout nombre inférieur à $2^n$ s'écrit comme une somme de puissances de $2$ inférieures, qui sont bien des diviseurs de $2^n$. 
 
 \emph{En pratique}, écrivons par exemple 1243 comme somme de diviseurs de $2^{11}=2048$. On commence par faire la liste des diviseurs de 2048 : 1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024.
 
 Dans 1243, on peut mettre 1024, il reste 219. \\
 Dans 219, on peut mettre 128, il reste 91.\\
 Dans 91, on peut mettre 64, il reste 27.\\
 Dans 27, on peut mettre 16, il reste 11.\\
 Dans 11, on peut mettre 8, il reste 3. \\
 Dans 3, on peut mettre 2, il reste 1. \\
 Dans 1, on peut mettre 1, il reste 0.\\
 Si on tire le bilan, on a donc 
 \[1243= 1+ 2+ 8+ 16+ 64+ 128+ 1024\]
